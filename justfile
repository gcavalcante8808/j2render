...
e2e-test: build-amd64
    cp -R renderer/tests/assets data/
    docker run -it --rm -v $(pwd)/data:/data --workdir /data debian:11-slim ./j2render_static.linux.amd64 -c assets/config.yaml

e2e-arm64-test: build-arm64
    cp -R renderer/tests/assets data/
    docker run -it --rm -v $(pwd)/data:/data --workdir /data alpine ./j2render_static.linux.arm64 -c assets/config.yaml
